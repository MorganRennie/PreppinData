-- FUNCTIONS USED:
-- CREATE SCHEMA, CREATE TABLE, CREATE TABLE AS, INSERT INTO, CAST, SUBSTRING_INDEX, CASE, WHERE
...
CREATE TABLE `WEEK1_OUTPUT1` AS
	...
	--FLIGHT_NUMBER
   		SUBSTRING_INDEX(
			SUBSTRING_INDEX(`Flight Details`,'//',2),'//',-1) AS 'FLIGHT_NUMBER',
	--FROM
    		SUBSTRING_INDEX(
		SUBSTRING_INDEX(
			SUBSTRING_INDEX(`Flight Details`,'//',3),
		'//',-1)
		, '-',1) AS 'FROM',
	--TO
		SUBSTRING_INDEX(
...
-- 	WHERE `Flow Card?`="1"; FOR "YES" VALUES, OR THE FOLLOWING FOR "NO" VALUES
	WHERE `Flow Card?`="0";
